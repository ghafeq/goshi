# @epds/enterprise-typography

`ent.typography.*` — **intentionally empty for now.**

Per the EPDS brief, the Enterprise type scale must come from its own Figma
type system once one exists — it must **not** be a copy of
[`@epds/consumer-typography`](../../consumer/typography)'s expressive scale.
...
2. Populate `src/type-scale.json` following the same token shape pattern as
   [`@epds/consumer-typography`'s `src/type-scale.json`](../../consumer/typography/src/type-scale.json)
...
3. Run `pnpm tokens:build`. No changes to the build script, package
   structure, or `@epds/enterprise-tokens`' consumption of this package are
   required.
