...
[`@goshi/enterprise-typography`](../typography) (currently empty — see that
package's README).

```ts
import { entByMode } from '@goshi/enterprise-tokens';
...
[`@goshi/enterprise-nextjs`](../nextjs)'s `ThemeProvider` / `useTheme()`.

## What's aliased vs. Enterprise-only

Colour, spacing, sizing, radius, elevation and motion are 1:1 aliases of the
shared foundation — identical values to `@goshi/consumer-tokens`' `con.*`
...
Note: `@goshi/enterprise-tokens` does **not** depend on `@goshi/consumer-tokens`
